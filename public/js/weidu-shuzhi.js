...
//切换三种图表：点击的时候移除和添加class
function clickclass(){
	$(this).parent().siblings().children('.iconfont').removeClass('clicking');
	$(this).addClass('clicking');
}
// 切换三种图表：初始化option为option1
function init_ption(option,chart,type){
	option = option1;
...
// 修改图表类型
	...
	$('.icon-iconfontcolor56').click(function(){
		// 设置点击的时候
		clickclass();
		init_ption(option1,myChart,"line");
	...
	$('.icon-62').click(function(){
		...
		init_ption(option1,myChart,"bar");
		// 判断维度和数值是否为空，如果为空，则不能切换图表
		ifdisabled(option);
		// 设置颜色属性的显示与隐藏
		$('.yanse1-chose').show();
	...
	$('.icon-sandian').click(function(){
		...
		init_ption(option1,myChart,"scatter");
